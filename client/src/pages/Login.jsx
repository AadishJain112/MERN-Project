...
const Login = () => {
  ...
  const handleRegisterSubmit = async (event) => {
    ...
    const requiredFields = ['name', 'email', 'password', 'department', 'batch', 'phone'];
    ...
    for (const field of requiredFields) {
      if (!registerForm[field] || !registerForm[field].trim()) {
        const fieldName = field === 'confirmPassword' ? 'Password confirmation' : field.charAt(0).toUpperCase() + field.slice(1).replace(/([A-Z])/g, ' $1');
  ...
  const switchMode = (newMode) => {
    ...
    setError('');
    if (newMode === 'register') {
      setRegisterForm((prev) => ({ ...prev, role: 'student', registrationNumber: '', semester: '', employeeId: '', designation: '' }));
    }
  };
        ...
        <div className="bg-white/95 px-10 py-12 text-slate-900">
          ...
          {mode === 'login' ? (
            ...
            <form onSubmit={handleRegisterSubmit} className="max-h-[70vh] overflow-y-auto">
              <div>
                <p className="accent-pill bg-slate-100 text-indigo-500">Create Account</p>
                <h3 className="mt-4 text-2xl font-semibold text-slate-900">Join CD-STAR</h3>
                <p className="text-sm text-slate-500">Register as a student or teacher.</p>
              </div>
              ...
              <div className="mt-6 space-y-4">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <div>
                    <label className="text-xs uppercase tracking-[0.3em] text-slate-500">Batch</label>
                    <input
                      type="text"
                      name="batch"
                ...
                {registerForm.role === 'student' && (
                  <>
                    <div>
                      <label className="text-xs uppercase tracking-[0.3em] text-slate-500">Registration Number</label>
                      <input
                        type="text"
                ...
                {registerForm.role === 'teacher' && (
                  <>
                    <div>
                      <label className="text-xs uppercase tracking-[0.3em] text-slate-500">Employee ID</label>
                      <input
                        type="text"
                        name="employeeId"
                    ...
                    <div>
                      <label className="text-xs uppercase tracking-[0.3em] text-slate-500">Designation</label>
                      <input
                        type="text"
                        name="designation"
              ...
              <button
                ...
                className="mt-8 w-full rounded-2xl bg-indigo-600 px-4 py-3 text-sm font-semibold text-white shadow-lg shadow-indigo-500/40 transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:bg-indigo-400"
              >
                {loading ? 'Creating account...' : `Register as ${registerForm.role}`}
              </button>
            </form>
          )}
        </div>
