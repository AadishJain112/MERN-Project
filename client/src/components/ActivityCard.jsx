...
const ActivityCard = ({ activity, actions }) => {
  ...
  const downloadAttachment = async (filePath) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const msg = data?.message || res.statusText || "Unable to fetch file";
        alert(`Unable to fetch attachment: ${msg}`);
        return;
      }
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl border border-white/70 bg-white p-5 shadow-xl shadow-slate-900/5">
      <div
        className={`absolute inset-y-0 left-0 w-1 bg-gradient-to-b ${palette.border}`}
      />
      <div className="relative flex flex-col gap-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <span
            className={`rounded-full px-3 py-1 text-xs font-semibold capitalize ${palette.badge}`}
          >
            {activity.status}
          </span>
        </div>
        ...
        {activity.file && (
          <button
            type="button"
            onClick={() => downloadAttachment(activity.file)}
